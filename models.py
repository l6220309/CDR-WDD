...
from center_loss_function import CenterLossLayer



def adv_loss_for_WGAN(y_true, y_pred):
    #return K.mean(y_true * y_pred)
    #return K.mean(-1.0 * y_true * y_pred)
    #return K.mean(-1.0 * y_true * y_pred)
    return K.mean(K.square(y_pred - y_true), axis=-1)

def d_loss_for_WGAN(y_true, y_pred):
    #return K.mean(-1.0 * y_true * y_pred)
    #return K.mean(-1.0 * y_true * y_pred)
    return K.mean(K.square(y_pred - y_true), axis=-1)
...
def ATCNet(n_classes, in_chans = 22, in_samples = 1125, n_windows = 3, attention = None, 
    ...
    input_2 = Permute((3,2,1))(input_1)                 #     TensorShape([None, 1125, 22, 1])
    regRate = 0.25
    numFilters = eegn_F1                    #16个卷积核
    F2 = numFilters*eegn_D                  #eegn_D: depth-wisej卷积的参数
    ...
    for i in range(n_windows): #5个窗口
        st = i   #st:start开始，block1.shape[1] = 20，5个窗口，i从1到5。   20 - 5 + i + 1 = 16 + i，每次窗口左移一位，共移动5次
        #几个窗口的数值:[0-16] [1-17] [2-18] [3-19] [4-20]
        end = block1.shape[1]-n_windows+i+1
        block2 = block1[:, st:end, :]   #取出0-16个变量，每一个长度32
...
#%% 总的模型
#TODO:把生成器模型的参数全部往这个函数里面扔
def Generator_Containing_Discriminator_Multiple_Outputs(generator, discriminator, in_chans=22, in_samples=1125, n_classes=4):

    input_s = Input(shape=(1, in_chans, in_samples))   # TensorShape([None, 1, 22, 1125])
    input_t = Input(shape=(1, in_chans, in_samples))   # TensorShape([None, 1, 22, 1125])

    # 生成器前向传播

    # prob_s, prob_t用来计算分类误差
    prob_s, latent_feat_s, prelogits_for_cent_loss_s = generator(input_s)
    prob_t, latent_feat_t, prelogits_for_cent_loss_t = generator(input_t)


    # prelogits_for_cent_loss_t.shape[0] = 16
    # 插入一个自定义的中心损失层，仅仅用来计算目标域的中心损失。
    center_loss_layer = CenterLossLayer(n_classes, prelogits_for_cent_loss_t.shape[1], input_shape=(16,), name='center_loss_layer')
    center_loss_layer_out = center_loss_layer(prelogits_for_cent_loss_t)

    # 鉴别器前向传播，validity代表特征来自源域（0）还是目标域（1）
    validity_s = discriminator(latent_feat_s)  # 对抗误差

    # validity_s用来计算对抗误差，prob_s和prob_t用来计算源域和目标域的分类误差，center_loss_layer_out计算目标域的中心误差。
    model = Model(inputs=[input_s, input_t], outputs=[validity_s, prob_s, prob_t, center_loss_layer_out], name="Combined")
...
#%% 生成器
def ATCNet_Generator(n_classes, in_chans = 22, in_samples = 1125, n_windows = 3, attention = None,
           eegn_F1 = 16, eegn_D = 2, eegn_kernelSize = 64, eegn_poolSize = 8, eegn_dropout=0.3,
           tcn_depth = 2, tcn_kernelSize = 4, tcn_filters = 32, tcn_dropout = 0.3,
           tcn_activation = 'elu', fuse = 'average'):
    ...
    regRate = 0.25
    numFilters = eegn_F1                    # 16个卷积核
    F2 = numFilters*eegn_D                  # eegn_D: depth-wise卷积的参数
    ...
    # Sliding window
    sw_concat = []   # to store concatenated sliding window outputs
    for i in range(n_windows):
        ...
        # Outputs of sliding window: concatenate_then_dense
        if i == 0:
            sw_concat = block3
        else:
            sw_concat = Concatenate()([sw_concat, block3])
    ...
    # sw_concat 维度 160
    # TODO: regRate是什么：防止过拟合的东西，记得查完记笔记
    net_1_latent_feature = Dense(64, kernel_constraint=max_norm(regRate))(sw_concat)
    # =============================以下是分类器==============================
    prelogits_for_cent_loss = Dense(16, kernel_constraint=max_norm(regRate))(net_1_latent_feature)
    predict = Dense(n_classes, kernel_constraint=max_norm(regRate))(prelogits_for_cent_loss)
    prob = Activation('softmax', name='softmax')(predict)

    return Model(inputs=input_1, outputs=[prob, net_1_latent_feature, prelogits_for_cent_loss], name="Generator")


# %% 鉴别器
def ATCNet_Discriminator():

    weight_init = RandomNormal(mean=0., stddev=0.02)

    input = Input(shape=(64,))   # TensorShape([None, 64])。64正好是来自特征提取器的特征长度
    regRate = 0.25

    fc1 = Dense(64, kernel_constraint=max_norm(regRate))(input)
    fc1_relued = LeakyReLU(alpha=0.05)(fc1)

    fc2 = Dense(32, kernel_constraint=max_norm(regRate))(fc1_relued)
    fc2_relued = LeakyReLU(alpha=0.05)(fc2)


    fc3 = Dense(16, kernel_constraint=max_norm(regRate))(fc2_relued)
    fc3_relued = LeakyReLU(alpha=0.05)(fc3)

    d_out_logits = Dense(1, kernel_constraint=max_norm(regRate))(fc3_relued)
    # 应用sigmoid函数可以将输出压缩至0～1的范围，但如果是WGAN，则直接输入logits
    #d_out_prob = Activation('sigmoid')(d_out)

    return Model(inputs=input, outputs=d_out_logits, name='Discriminator')
...
def TCN_block(input_layer,input_dimension,depth,kernel_size,filters,dropout,activation='relu'):
    ...
    """    
    #32个过滤器，kersize 4，dilation rate = 1 ，casual padding
...
def ShallowConvNet(nb_classes, Chans = 64, Samples = 128, dropoutRate = 0.5):
    ...
    block1 = Conv2D(40, (1, 13), input_shape=(Chans, Samples, 1), kernel_constraint=max_norm(2., axis=(0, 1, 2)))(input_2)
...
def ATCNet_classifier(n_classes, Chans=22, Samples=1125, F1=8, D=2, kernLength=64, dropout_eeg=0.25):
    input1 = Input(shape=(1, Chans, Samples))
    input2 = Permute((3, 2, 1))(input1)
    regRate=.25
